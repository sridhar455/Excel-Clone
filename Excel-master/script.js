...
fileOptions.addEventListener("click", (e) => {
  ...
  if (e.currentTarget.classList.length == 0) {
    ...
    let allFilesOption = e.currentTarget.querySelectorAll("span span");
    // 1.) clear
    allFilesOption[0].addEventListener("click", function () {
      let allCellls = document.querySelectorAll(".cell");
      for (let i = 0; i < allCellls.length; i++) {

        allCellls[i].innerText = "";
        
        let cellAddress = allCellls[i].getAttribute("data-address");
        dataObj[cellAddress] = {
          value: "",
          formula: "",
          upstream: [],
    ...
    // 2.) Open
    allFilesOption[1].addEventListener("click", function () {
      // a.) fetch from dataObj from localStorage
      // b.) Replace current dataObj with the fetched Obj 

      dataObj = JSON.parse(localStorage.getItem("sheet"));

      // c.) Populate UI with this data
      for(let j=1;j<=100;j++){
        for(let i=0;i<26;i++){
          let address = String.fromCharCode(i+65)+j;
          let cellObj = dataObj[address];
          console.log(cellObj+" "+address);
    ...
    // 3.) save
    allFilesOption[2].addEventListener("click", function () {
